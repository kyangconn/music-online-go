...
func TestTokenContract(t *testing.T) {
	...
	config.AppConfig = &config.Config{JWT: config.JWTConfig{
	...
	t.Run("generated token uses the accepted issuer and algorithm", func(t *testing.T) {
		token, err := GenerateToken(7, "listener", "user")
		if err != nil {
		...
		if claims.UserID != 7 || claims.Issuer != tokenIssuer {
	...
	t.Run("preserves the expired-token error", func(t *testing.T) {
		...
		if _, err := ParseToken(token); !errors.Is(err, ErrExpiredToken) {
