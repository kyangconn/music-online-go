// Package mediametadata reads bounded embedded tags and cover art from local
// audio files. It deliberately has no database or scan-job responsibilities.
...
const (
	...
	coverSignatureReadSize  = 512
)

var supportedCoverMIMEs = map[string]struct{}{
...
func Read(path string, maxTagBytes, maxCoverBytes int64) (*domain.CreateMusicRequest, *Cover, error) {
...
func boundedScannedCover(cover *Cover, maxCoverBytes int64) *Cover {
...
func bestFLACCover(pictures []*flac.Picture, maxCoverBytes int64) *Cover {
	var best *Cover
	for _, picture := range pictures {
		...
		candidate := &Cover{MIME: picture.MIME, Data: picture.Data, Type: pictureType}
...
func bestVorbisCover(comments *flac.VorbisComment, maxCoverBytes int64) *Cover {
...
func bestAPECover(items []ape.Item, maxCoverBytes int64) *Cover {
	var best *Cover
	for _, item := range items {
		...
		candidate := &Cover{Data: item.Value[separator+1:], Type: pictureType}
		best = preferScannedCover(best, candidate)
		if pictureType == tunetag.PictureCoverFront {
			break
		}
...
func bestMP4Cover(tag *mp4.Ilst, maxCoverBytes int64) *Cover {
	if tag == nil {
		return nil
	}
	for _, picture := range tag.Pictures() {
		...
		return &Cover{MIME: mime, Data: picture.Payload, Type: tunetag.PictureCoverFront}
	}
	return nil
}

func preferScannedCover(current, candidate *Cover) *Cover {
	if candidate == nil {
		return current
	}
...
func applyID3Tag(req *domain.CreateMusicRequest, tag *id3v2.Tag) *Cover {
	if tag == nil {
		return nil
	}
	if req.Title == "" {
	...
	for _, picture := range tag.PictureFrames() {
		...
		candidate := &Cover{MIME: picture.MIME, Data: picture.Data, Type: tunetag.PictureType(picture.PictureType)}
...
func readASFMetadata(file *os.File, maxTagBytes, maxCoverBytes int64) (*domain.CreateMusicRequest, *Cover, error) {
	req := &domain.CreateMusicRequest{}
	info, err := file.Stat()
	if err != nil {
		return req, nil, err
	...
	for _, picture := range parsed.Pictures() {
		...
		candidate := &Cover{MIME: picture.MIME, Data: picture.Data, Type: tunetag.PictureType(picture.Type)}
...
func sanitizeScannedMetadata(req *domain.CreateMusicRequest) {
	...
	req.Artists = boundedMetadataValues(musicmeta.NormalizeDisplayValues(req.Artists, false), 255)
	req.AlbumArtists = boundedMetadataValues(musicmeta.NormalizeDisplayValues(req.AlbumArtists, false), 255)
	req.Genres = boundedMetadataValues(musicmeta.NormalizeDisplayValues(req.Genres, false), 500)
	if req.Artist == "" && len(req.Artists) > 0 {
		req.Artist = musicmeta.JoinDisplayValues(req.Artists, 255)
	}
	if req.AlbumArtist == "" && len(req.AlbumArtists) > 0 {
		req.AlbumArtist = musicmeta.JoinDisplayValues(req.AlbumArtists, 255)
	}
...
func boundedMetadataValues(values domain.StringList, maxBytes int) domain.StringList {
	if len(values) > musicmeta.MaxCanonicalMetadataValues {
		values = values[:musicmeta.MaxCanonicalMetadataValues]
	}
...
func safePartialDate(value string) string {
	...
	if musicmeta.ValidatePartialDate("date", value) != nil {
		return ""
	}
	return value
}
...
func validISRCValues(values domain.StringList) domain.StringList {
	...
	for _, value := range values {
		for _, candidate := range splitTagList(value) {
			normalized, err := musicmeta.NormalizeISRCs(domain.StringList{candidate})
			if err == nil {
...
func validMBIDValue(value string) string {
	normalized, err := musicmeta.NormalizeMBID("musicbrainz_id", strings.TrimSpace(value))
	if err != nil {
		return ""
	}
	return normalized
...
func readWAVMetadata(file *os.File, maxTagBytes int64) (*domain.CreateMusicRequest, *Cover, error) {
	req := &domain.CreateMusicRequest{}
	if _, err := file.Seek(0, io.SeekStart); err != nil {
		return req, nil, err
	}
...
func readAIFFMetadata(file *os.File, maxTagBytes int64) (*domain.CreateMusicRequest, *Cover, error) {
	req := &domain.CreateMusicRequest{}
	if _, err := file.Seek(0, io.SeekStart); err != nil {
		return req, nil, err
	}
...
func SaveCover(musicID uint, picture *Cover, uploadDir string, maxCoverBytes int64) (string, error) {
	...
	detected := http.DetectContentType(picture.Data[:minInt(len(picture.Data), coverSignatureReadSize)])
	if _, ok := supportedCoverMIMEs[detected]; !ok {
