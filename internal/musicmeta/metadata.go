// Package musicmeta owns canonical metadata normalization shared by API
// writes and bounded file-tag ingestion.
package musicmeta
...
const MaxCanonicalMetadataValues = maxCanonicalMetadataValues

func ApplyCreate(music *domain.Music, req *domain.CreateMusicRequest) error {
	return applyCreateMusicMetadata(music, req)
}

func ApplyUpdate(music *domain.Music, req *domain.UpdateMusicRequest) (bool, error) {
	return applyUpdateMusicMetadata(music, req)
}

func Normalize(music *domain.Music) error {
	return normalizeMusicMetadata(music)
}

func NormalizeDisplayValues(values domain.StringList, split bool) domain.StringList {
	return normalizeDisplayValues(values, split)
}

func JoinDisplayValues(values domain.StringList, maxBytes int) string {
	return joinMetadataDisplayValues(values, maxBytes)
}

func NormalizeISRCs(values domain.StringList) (domain.StringList, error) {
	return normalizeISRCs(values)
}

func NormalizeMBID(field, value string) (string, error) {
	return normalizeMBID(field, value)
}

func ValidatePartialDate(field, value string) error {
	return validatePartialDate(field, value)
}

func FromMusic(music *domain.Music) domain.MusicMetadata {
	return musicMetadataFromMusic(music)
}
